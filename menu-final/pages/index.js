...
function FullMenuUploader({ value = [], onChange }) {
  ...
  async function handleFiles(e) {
    ...
    if (!selected.length) {
      alert('Можеш да качиш максимум 30 файла за цялото меню.');
      e.target.value = '';
      return;
    }
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      >
        {uploading ? '⏳ Качване...' : `📚 Качи файлове за цялото меню (${(value || []).length}/30)`}
      </button>
...
export default function Home() {
  ...
  const [fullMenuPage, setFullMenuPage] = useState(0);
  ...
  function addSectionAfter(si) {
    const m = JSON.parse(JSON.stringify(adminMenu));
    m.sections.splice(si + 1, 0, { label: 'Нова категория', dishes: [] });
    setAdminMenu(m);
  }
  ...
  return (
    <>
        ...
        <div style={{ background: bg, minHeight: '100vh', transition: 'background 0.3s' }}>
          ...
          {Array.isArray(menu.fullMenuImages) && menu.fullMenuImages.length > 0 && (
            <div style={{ padding: '1rem 1.25rem 0' }}>
              <button
                onClick={() => { setFullMenuPage(0); setScreen('fullMenu'); }}
                style={{
                  width: '100%',
                  padding: '12px 14px',
                  borderRadius: 12,
      ...
      {screen === 'fullMenu' && menu && (() => {
        const pages = menu.fullMenuImages || [];
        const total = pages.length;
        const goTo = (n) => setFullMenuPage(((n % total) + total) % total);
        ...
        function onTouchEnd(e) {
          ...
          if (dx > 40) goTo(fullMenuPage - 1);
          else if (dx < -40) goTo(fullMenuPage + 1);
          touchStartX = null;
        }

        return (
          <div style={{ background: dark ? '#0c0a06' : '#15110a', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(12,10,6,0.85)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(184,134,11,0.25)', padding: '0.9rem 1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              <button onClick={() => setScreen('menu')} style={{ background: 'none', border: '1px solid rgba(245,230,163,0.28)', color: '#F5E6A3', padding: '7px 12px', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>
                ← Назад
              </button>

              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#e9c86f', fontSize: 13, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase' }}>
                  Цялото меню
                </div>
                <div style={{ color: 'rgba(245,230,163,0.55)', fontSize: 11, marginTop: 2, letterSpacing: '0.05em' }}>
                  Страница {fullMenuPage + 1} от {total}
                </div>
              </div>
            ...
            >
              {total > 1 && (
                <button
                  onClick={() => goTo(fullMenuPage - 1)}
                  aria-label="Предишна страница"
                  style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', zIndex: 5, width: 40, height: 40, borderRadius: '50%', border: '1px solid rgba(245,230,163,0.3)', background: 'rgba(12,10,6,0.55)', color: '#F5E6A3', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)' }}
                >
                  ‹
                </button>
              )}
              ...
              <div
                ...
                style={{
                  ...
                  overflow: 'hidden',
                  background: '#1c1810',
                  boxShadow: '0 24px 60px rgba(0,0,0,0.55), 0 2px 0 rgba(245,230,163,0.06) inset',
                  border: '1px solid rgba(184,134,11,0.35)',
                  animation: 'fadeUp 0.35s ease both'
                }}
              >
                <img
                  src={pages[fullMenuPage]}
                  alt={`Цялото меню страница ${fullMenuPage + 1}`}
              ...
              {total > 1 && (
                <button
                  ...
                  style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', zIndex: 5, width: 40, height: 40, borderRadius: '50%', border: '1px solid rgba(245,230,163,0.3)', background: 'rgba(12,10,6,0.55)', color: '#F5E6A3', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)' }}
                >
                  ›
                </button>
              )}
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 6, padding: '4px 0 22px', flexWrap: 'wrap', maxWidth: 480, margin: '0 auto' }}>
                {pages.map((_, i) => (
                  <button
                    ...
                    style={{
                      width: i === fullMenuPage ? 20 : 6,
                      height: 6,
                      borderRadius: 3,
                      border: 'none',
                      cursor: 'pointer',
                      transition: 'width 0.25s ease, background 0.25s ease',
                      background: i === fullMenuPage ? '#e9c86f' : 'rgba(245,230,163,0.28)'
                    }}
                  />
                ))}
              </div>
      ...
      {screen === 'admin' && adminMenu && (
        <div style={{ background: '#FDFAF5', minHeight: '100vh' }}>
          <div style={{ background: '#1A1208', color: '#F5E6A3', padding: '1.25rem', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ marginLeft: 'auto', fontSize: 12, color: 'rgba(245,230,163,0.75)' }}>
          ...
          <div style={{ padding: '1.25rem' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, color: '#B8860B', marginBottom: 6 }}>ЦЯЛО МЕНЮ — ДО 30 ФАЙЛА</div>
                <FullMenuUploader value={adminMenu.fullMenuImages || []} onChange={imgs => setAdminMenu({ ...adminMenu, fullMenuImages: imgs })} />
              </div>
            </div>

            <div style={{ marginBottom: '1.5rem', background: 'white', border: '0.5px solid rgba(184,134,11,0.2)', borderRadius: 10, padding: '0.9rem 1rem' }}>
              <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B8860B', marginBottom: 10 }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '8px 12px', border: '0.5px solid rgba(184,134,11,0.2)', borderRadius: 8, fontSize: 14, background: '#FDFAF5', outline: 'none', marginBottom: 8 }}
              />
              ...
              <button
                ...
                disabled={passSaving}
                style={{ background: '#1A1208', color: '#F5E6A3', border: 'none', padding: '8px 16px', borderRadius: 8, fontSize: 13, cursor: 'pointer', opacity: passSaving ? 0.6 : 1 }}
              >
              ...
              <div key={si} style={{ marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <input
                    ...
                    style={{ flex: 1, fontSize: 12, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#B8860B', padding: '6px 8px', border: '0.5px solid rgba(184,134,11,0.25)', borderRadius: 6, background: 'white', outline: 'none' }}
                  />
                ...
                <button
                  ...
                  style={{ width: '100%', marginTop: 8, padding: 7, border: '0.5px dashed rgba(184,134,11,0.3)', borderRadius: 8, background: 'transparent', color: '#B8860B', fontSize: 12, cursor: 'pointer' }}
                >
