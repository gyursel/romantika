...
function FullMenuUploader({ value = [], onChange }) {
  ...
  async function handleFiles(e) {
    ...
    const freeSlots = Math.max(0, 16 - current.length);
    const selected = files.slice(0, freeSlots);

    if (!selected.length) {
      alert('Можеш да качиш максимум 16 файла за цялото меню.');
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      onChange([...current, ...urls].slice(0, 16));
    } catch (err) {
      alert('Грешка при качване: ' + err.message);
    }
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <button
        ...
        disabled={uploading || (value || []).length >= 16}
        style={{
          padding: '8px 12px',
      ...
      >
        {uploading ? '⏳ Качване...' : `📚 Качи файлове за цялото меню (${(value || []).length}/16)`}
      </button>
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '0.875rem 1rem', display: 'flex', justifyContent: 'space-between', gap: 8 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 500, color: dark ? '#d4a830' : '#7A5C00', whiteSpace: 'nowrap' }}>
          {d.price} €
        </div>
      </div>
    </div>
          ...
          <div style={{ padding: '1.25rem' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, color: '#B8860B', marginBottom: 6 }}>ЦЯЛО МЕНЮ — ДО 16 ФАЙЛА</div>
