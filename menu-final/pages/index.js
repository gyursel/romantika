...
function MultiImageUploader({ values, onChange, max = 8 }) {
  const inputRef = useRef();
  const [uploading, setUploading] = useState(false);
  ...
  async function handleFiles(e) {
    ...
    if (toUpload.length < files.length) {
      alert(`Може да качиш максимум ${max} снимки общо. Ще се качат първите ${toUpload.length}.`);
    }
    setUploading(true);
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
        {list.map((url, i) => (
          <div key={i} style={{ position: 'relative', display: 'inline-block' }}>
            <img src={url} alt="" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 8, border: '0.5px solid rgba(184,134,11,0.3)', display: 'block' }} />
            <button onClick={() => removeAt(i)} style={{ position: 'absolute', top: 3, right: 3, background: 'rgba(226,75,74,0.9)', border: 'none', color: 'white', fontSize: 13, width: 18, height: 18, borderRadius: '50%', lineHeight: '18px', textAlign: 'center', padding: 0, cursor: 'pointer' }}>×</button>
          </div>
        ))}
      </div>
      {list.length < max && (
        <button onClick={() => inputRef.current.click()} disabled={uploading}
          style={{ padding: '8px 14px', border: '0.5px dashed rgba(184,134,11,0.4)', borderRadius: 8, background: 'transparent', color: uploading ? '#aaa' : '#B8860B', fontSize: 13, cursor: uploading ? 'default' : 'pointer' }}>
          {uploading ? `⏳ Качване ${progress.done}/${progress.total}...` : `📷 Добави снимки (${list.length}/${max})`}
        </button>
      )}
      {list.length >= max && (
        <div style={{ fontSize: 12, color: '#B8860B', opacity: 0.8 }}>Достигнат е максимумът от {max} снимки.</div>
      )}
    </div>
  );
}
    ...
    <>
      ...
      {screen === 'menu' && menu && (
        <div style={{ background: bg, minHeight: '100vh', transition: 'background 0.3s' }}>
          ...
          {menu.menuGallery?.length > 0 && (
            <div style={{ padding: '0 1.25rem 1.5rem' }}>
              <div className="section-label" style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#B8860B', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                Цялото меню
                <div style={{ flex: 1, height: 0.5, background: dark ? 'rgba(184,134,11,0.15)' : 'rgba(184,134,11,0.2)' }} />
              </div>
              <div style={{ display: 'flex', overflowX: 'auto', gap: 10, paddingBottom: 6, WebkitOverflowScrolling: 'touch' }}>
                {menu.menuGallery.map((url, i) => (
                  <img key={i} src={url} alt={`Меню стр. ${i + 1}`}
                    onClick={() => window.open(url, '_blank')}
                    style={{ width: 140, height: 190, objectFit: 'cover', borderRadius: 10, flexShrink: 0, border: `0.5px solid ${dark ? 'rgba(184,134,11,0.25)' : 'rgba(184,134,11,0.2)'}`, cursor: 'pointer' }} />
                ))}
              </div>
            </div>
          )}
          ...
          <div style={{ padding: '1.25rem' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ marginTop: 16 }}>
                <div style={{ fontSize: 11, color: '#B8860B', marginBottom: 6 }}>СНИМКИ НА ЦЯЛОТО МЕНЮ (до 8 бр.)</div>
                <MultiImageUploader values={adminMenu.menuGallery || []} onChange={list => setAdminMenu({...adminMenu, menuGallery: list})} max={8} />
              </div>
            </div>
