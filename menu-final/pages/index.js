...
function DishCard({ d, dark, badgeColors, index }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: dark ? '0 0 8px rgba(184,134,11,0.35)' : '0 0 6px rgba(184,134,11,0.2)',
      borderRadius: 12,
      marginBottom: 8,
      overflow: 'hidden',
      opacity: visible ? 1 : 0,
...
export default function Home() {
  ...
  return (
    <>
      ...
      {showFullMenu && menu?.fullMenuImage && (
        <div onClick={() => setShowFullMenu(false)}
          style={{
          ...
          <button onClick={() => setShowFullMenu(false)}
            style={{
              position: 'absolute', top: 16, right: 16,
              background: 'rgba(184,134,11,0.15)',
              border: '1px solid rgba(184,134,11,0.5)',
              color: '#F5E6A3', fontSize: 18, width: 36, height: 36,
              borderRadius: '50%', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>×</button>
          <div onClick={e => e.stopPropagation()}
            style={{ maxWidth: '100%', maxHeight: '100%', overflow: 'auto', borderRadius: 12, border: '1.5px solid #B8860B', boxShadow: '0 0 20px rgba(184,134,11,0.4)' }}>
            <img src={menu.fullMenuImage} alt="Пълно меню" style={{ display: 'block', width: '100%', height: 'auto' }} />
          </div>
        </div>
      )}
          ...
          <div style={{ padding: '1.25rem' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, color: '#B8860B', marginBottom: 6 }}>СНИМКА НА ЦЯЛОТО МЕНЮ (за бутон "Виж цялото меню")</div>
                <ImageUploader value={adminMenu.fullMenuImage || ''} onChange={url => setAdminMenu({...adminMenu, fullMenuImage: url})} />
              </div>
            </div>
