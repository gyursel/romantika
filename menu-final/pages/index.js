...
import { doc, getDoc, setDoc, collection, getDocs, addDoc, onSnapshot, increment } from 'firebase/firestore';
...
function DishCard({ d, dark, badgeColors, index }) {
  ...
  return (
    <div style={{
      background: dark ? '#2b2419' : '#EAD9B8',
      border: `1px solid ${dark ? 'rgba(212,168,58,0.4)' : 'rgba(184,134,11,0.45)'}`,
      boxShadow: dark ? '0 1px 0 rgba(212,168,58,0.06) inset' : '0 1px 0 rgba(255,255,255,0.35) inset',
      borderRadius: 12,
      marginBottom: 8,
      overflow: 'hidden',
      opacity: visible ? 1 : 0,
      ...
      <div style={{ padding: '0.875rem 1rem', display: 'flex', justifyContent: 'space-between', gap: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: dark ? '#FBEFC2' : '#241708', marginBottom: 3 }}>{d.name}</div>
          {d.desc && <div style={{ fontSize: 12, color: dark ? '#cbb588' : '#6B5738', lineHeight: 1.5 }}>{d.desc}</div>}
          {d.weight && <div style={{ fontSize: 11, color: dark ? '#B8860B' : '#9C6B1F', marginTop: 4, opacity: 0.85 }}>⚖ {d.weight}</div>}
        ...
        <div style={{ fontSize: 17, fontWeight: 700, color: dark ? '#d4a830' : '#8A5A16', whiteSpace: 'nowrap' }}>
          {d.price} €
        </div>
      </div>
    </div>
    ...
    <div
      style={{
        ...
        background: dark
          ...
          : 'linear-gradient(135deg, #EAD9B8, #F5EEDF)',
...
export default function Home() {
  ...
  function playMenuChangeChime() {
    try {
      ...
      // тих fallback, ако AudioContext не е наличен/позволен
    }
  }

  function triggerMenuChangeSignal() {
    playMenuChangeChime();
    setMenuFlash(true);
    setTimeout(() => setMenuFlash(false), 1700);

    if (screenRef.current !== 'admin') {
      showToast('Менюто е обновено 🔔');
    }
  }
    ...
    const unsubscribe = onSnapshot(
      ...
      snap => {
        if (snap.exists()) {
          ...
          // Пропускаме сигнала при първоначалното зареждане на страницата —
          // звук и цвят се задействат само при реална промяна, докато менюто вече е отворено.
          if (!firstSnapshot) triggerMenuChangeSignal();
        ...
        if (firstSnapshot) finishInitialLoad();
      },
      () => {
        if (firstSnapshot) {
          setMenu(defaultMenu);
  ...
  useEffect(() => {
    ...
    const statsRef = doc(db, 'stats', todayKey);

    // Броим всяко влизане в сайта (не само веднъж на ден на устройство).
    setDoc(statsRef, { count: increment(1), date: todayKey }, { merge: true }).catch(() => {});

    const unsubscribe = onSnapshot(
      statsRef,
      snap => setVisitorCount(snap.exists() ? (snap.data().count || 0) : 0),
      () => setVisitorCount(0)
    );
    ...
    <>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 520px) {
          .hero-corner-badge { font-size: 9.5px !important; padding: 3px 6px !important; }
        }
      ...
      {screen === 'menu' && menu && (
        <div style={{ background: bg, minHeight: '100vh', transition: 'background 0.3s' }}>
          {menu.phone && (
            <div style={{ background: '#9C6B1F', color: '#1A1208', textAlign: 'center', padding: '7px 1rem', fontSize: 13, fontWeight: 600, letterSpacing: '0.03em', animation: 'fadeDown 0.4s ease' }}>
          ...
          >
            ...
            <div
              ...
              style={{ position: 'absolute', zIndex: 2, bottom: 10, left: 10, background: 'rgba(14,9,4,0.46)', border: '1px solid rgba(234,201,111,0.42)', color: '#f2d98c', fontSize: 10.5, padding: '4px 8px', borderRadius: 6, backdropFilter: 'blur(4px)', whiteSpace: 'nowrap' }}
            >
              📅 {todayLabel}
            </div>
            ...
            <div
              className="hero-corner-badge"
              style={{ position: 'absolute', zIndex: 2, bottom: 10, right: 10, background: 'rgba(14,9,4,0.46)', border: '1px solid rgba(234,201,111,0.42)', color: '#f2d98c', fontSize: 10.5, padding: '4px 8px', borderRadius: 6, backdropFilter: 'blur(4px)', whiteSpace: 'nowrap' }}
            >
          ...
          {Array.isArray(menu.fullMenuImages) && menu.fullMenuImages.length > 0 && (
            <div style={{ padding: '1rem 1.25rem 0' }}>
              <button
                ...
                onMouseDown={() => setMenuBtnPressed(true)}
                onMouseUp={() => setMenuBtnPressed(false)}
                onMouseLeave={() => setMenuBtnPressed(false)}
                onTouchStart={() => setMenuBtnPressed(true)}
                onTouchEnd={() => setMenuBtnPressed(false)}
                onTouchCancel={() => setMenuBtnPressed(false)}
                className="btn-viewmenu"
                style={{
                  border: dark ? '1px solid rgba(212,168,58,0.55)' : '1px solid rgba(184,134,11,0.5)',
                  background: dark
                    ? 'linear-gradient(160deg, #3d2c11 0%, #1c1408 100%)'
                    : 'linear-gradient(160deg, #fbf1d8 0%, #ecd9a8 100%)',
                  color: dark ? '#F5E6A3' : '#5c3d0a',
                  transform: menuBtnPressed ? 'translateY(4px) scale(0.99)' : 'translateY(0) scale(1)',
                  boxShadow: menuBtnPressed
                    ? (dark
                        ? '0 1px 0 #6b4a12, 0 3px 10px rgba(0,0,0,0.4)'
                        : '0 1px 0 #B8860B, 0 3px 10px rgba(122,92,0,0.2)')
                    : (dark
                        ? '0 5px 0 #6b4a12, 0 14px 26px rgba(0,0,0,0.45)'
                        : '0 5px 0 #B8860B, 0 12px 24px rgba(122,92,0,0.22)')
                }}
              >
                <span
                  className="btn-viewmenu-icon"
                  style={{ background: dark ? 'rgba(245,230,163,0.14)' : 'rgba(92,61,10,0.10)' }}
                >
                  📖
                </span>
                Виж цялото меню
            ...
            {(menu.sections || []).map((sec, si) => {
              ...
              return (
                <div key={si} style={{ marginBottom: '1.75rem', animationDelay: `${si * 0.1}s` }}>
                  <div className="section-label" style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.15em', textTransform: 'uppercase', color: dark ? '#B8860B' : '#9C6B1F', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8, animationDelay: `${0.2 + si * 0.1}s` }}>
                    {sec.label}
                    <div style={{ flex: 1, height: 0.5, background: dark ? 'rgba(184,134,11,0.15)' : 'rgba(78,124,110,0.3)' }} />
                  </div>
        ...
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: bg }}>
          ...
          <button
            ...
            style={{
              ...
              background: dark ? 'linear-gradient(135deg, rgba(184,134,11,0.20), rgba(43,36,25,0.96))' : 'linear-gradient(135deg, #EAD9B8, #F5EEDF)',
          ...
          <div style={{ padding: '1.25rem' }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10,
                border: '1px solid rgba(184,134,11,0.45)',
                background: saving ? '#555' : 'linear-gradient(135deg, rgba(184,134,11,0.20), rgba(43,36,25,0.96))',
                color: '#F5E6A3',
                fontSize: 15,
                fontWeight: 700,
                letterSpacing: '0.04em',
                ...
                boxShadow: saving ? 'none' : '0 10px 28px rgba(0,0,0,0.22)'
              }}
            >
      ...
      {menuFlash && (
        <div
          style={{
            ...
            zIndex: 500,
            pointerEvents: 'none',
            boxShadow: 'inset 0 0 0 5px #B8860B, inset 0 0 40px rgba(184,134,11,0.55)',
            animation: 'menuFlashPulse 1.7s ease',
          }}
        />
      )}
