  ...
  sections: [
    {
      label: 'Супа',
      dishes: [
        { name: 'Телешки бульон с фиде', desc: 'Домашен бульон, зеленчуци, магданоз', price: '4.50', badges: [] },
        { name: 'Крем супа от тиква', desc: 'Орехи, сметана, черен пипер', price: '5.00', badges: ['veg'] }
      ]
    },
    {
      label: 'Основно',
      dishes: [
        { name: 'Свинско с картофи на фурна', desc: 'Печено бавно, с розмарин и мащерка', price: '12.00', badges: [] },
        { name: 'Пилешко с гъби', desc: 'В маслено-чеснов сос, с ориз', price: '10.50', badges: ['new'] },
        { name: 'Гевречета с тахан', desc: 'Сезонни зеленчуци, хумус, кисело мляко', price: '9.00', badges: ['veg'] }
      ]
    },
    {
      label: 'Десерт',
      dishes: [
        { name: 'Баница с ябълки', desc: 'Хрупкава коричка, канела, пудра захар', price: '3.50', badges: [] },
        { name: 'Тирамису', desc: 'По класическа рецепта с маскарпоне', price: '5.50', badges: [] }
      ]
    }
  ]
};
...
export default function Home() {
  ...
  function addDish(si) {
    const m = JSON.parse(JSON.stringify(adminMenu));
    m.sections[si].dishes.push({ name: '', desc: '', price: '', badges: [] });
    setAdminMenu(m);
  }

  const inputStyle = { fontFamily: 'inherit', width: '100%', padding: '8px 12px', border: '0.5px solid rgba(184,134,11,0.2)', borderRadius: 8, fontSize: 14, background: 'white', outline: 'none', marginBottom: 8 };
  const smallInputStyle = { fontFamily: 'inherit', flex: 1, padding: '6px 8px', border: '0.5px solid rgba(184,134,11,0.2)', borderRadius: 4, fontSize: 13, background: '#FDFAF5', outline: 'none' };
  ...
  return (
    <>
      ...
      <style>{`* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #FDFAF5; } input, button, select { font-family: inherit; cursor: pointer; }`}</style>
        ...
        <div>
          <div style={{ background: '#1A1208', color: '#F5E6A3', padding: '2rem 1.25rem 1.5rem', textAlign: 'center', position: 'relative' }}>
            <button onClick={() => { setPassword(''); setLoginError(false); setScreen('login'); }}
              style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'rgba(245,230,163,0.1)', border: '0.5px solid rgba(245,230,163,0.3)', color: 'rgba(245,230,163,0.7)', fontSize: 11, padding: '4px 8px', borderRadius: 4 }}>
              ⚙ Админ
            </button>
          ...
          <div style={{ padding: '1.25rem', background: '#FDFAF5' }}>
            {menu.sections.map((sec, si) => (
              <div key={si} style={{ marginBottom: '1.75rem' }}>
                ...
                {sec.dishes.map((d, di) => (
                  <div key={di} style={{ background: 'white', border: '0.5px solid rgba(184,134,11,0.2)', borderRadius: 12, padding: '0.875rem 1rem', marginBottom: 8, display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 500, color: '#7A5C00', whiteSpace: 'nowrap' }}>{d.price} лв</div>
                  </div>
                ))}
              </div>
            ))}
      ...
      {screen === 'login' && (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: '#FDFAF5' }}>
          ...
          <button onClick={doLogin}
            style={{ width: '100%', maxWidth: 320, padding: 11, background: '#1A1208', color: '#F5E6A3', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500 }}>
            Влез
          </button>
          {loginError && <div style={{ color: '#E24B4A', fontSize: 12, marginTop: 8 }}>Грешна парола</div>}
          <button onClick={() => setScreen('menu')} style={{ marginTop: '1rem', background: 'none', border: 'none', color: '#6B5E3E', fontSize: 13 }}>← Назад към менюто</button>
        </div>
      )}
      ...
      {screen === 'admin' && adminMenu && (
        <div>
          <div style={{ background: '#1A1208', color: '#F5E6A3', padding: '1.25rem', display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={() => setScreen('menu')} style={{ background: 'none', border: '0.5px solid rgba(245,230,163,0.3)', color: 'rgba(245,230,163,0.8)', padding: '5px 10px', borderRadius: 4, fontSize: 12 }}>← Меню</button>
          ...
          <div style={{ padding: '1.25rem', background: '#FDFAF5', minHeight: '80vh' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <input value={adminMenu.footerNote} onChange={e => setAdminMenu({...adminMenu, footerNote: e.target.value})} placeholder="Бележка в долната част" style={{...inputStyle, marginBottom: 0}} />
            </div>
            ...
            {adminMenu.sections.map((sec, si) => (
              <div key={si} style={{ marginBottom: '1.5rem' }}>
                ...
                {sec.dishes.map((d, di) => (
                  <div key={di} style={{ background: 'white', border: '0.5px solid rgba(184,134,11,0.2)', borderRadius: 10, padding: '0.75rem 1rem', marginBottom: 8 }}>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                      <input value={d.name} onChange={e => updateDish(si, di, 'name', e.target.value)} placeholder="Ястие" style={smallInputStyle} />
                      <input value={d.price} onChange={e => updateDish(si, di, 'price', e.target.value)} placeholder="Цена" style={{...smallInputStyle, flex: 'none', width: 70}} />
                      <button onClick={() => deleteDish(si, di)} style={{ background: 'none', border: 'none', color: '#E24B4A', fontSize: 20, opacity: 0.6, padding: '0 4px' }}>×</button>
                    </div>
                    <input value={d.desc} onChange={e => updateDish(si, di, 'desc', e.target.value)} placeholder="Описание (по желание)" style={{...smallInputStyle, width: '100%', marginBottom: 8, display: 'block'}} />
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {['veg', 'spicy', 'new'].map(b => (
                        <button key={b} onClick={() => toggleBadge(si, di, b)}
                          style={{ fontSize: 10, padding: '3px 9px', borderRadius: 20, border: d.badges.includes(b) ? `0.5px solid ${BADGE_COLORS[b].color}` : '0.5px solid rgba(184,134,11,0.2)', background: d.badges.includes(b) ? BADGE_COLORS[b].bg : 'white', color: d.badges.includes(b) ? BADGE_COLORS[b].color : '#6B5E3E' }}>
                ...
                <button onClick={() => addDish(si)} style={{ width: '100%', padding: 8, border: '0.5px dashed rgba(184,134,11,0.3)', borderRadius: 8, background: 'transparent', color: '#B8860B', fontSize: 13 }}>
                  + Добави ястие
                </button>
              </div>
            ))}
            ...
            <button onClick={saveMenu} disabled={saving}
              style={{ width: '100%', padding: 13, background: saving ? '#555' : '#1A1208', color: '#F5E6A3', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 500, letterSpacing: '0.04em', marginTop: '0.5rem' }}>
